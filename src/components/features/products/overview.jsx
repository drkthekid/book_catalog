  ...
  return (
    <Dialog defaultOpen>
      <DialogContent
        ...
        <div
          className="max-md:hide-scrollbar grid overflow-auto max-md:h-full md:grid-cols-2 md:overflow-hidden">
          <div>
            <ProductImages images={images} />
          </div>
          <div>
            <div
              className="hide-scrollbar h-full space-y-5 px-8 py-8 md:h-[var(--dialog-height)] md:max-h-[var(--dialog-max-height)] md:overflow-auto md:px-15 md:py-14">
...
const ProductForm = ({
  ...
  const currentColor = useMemo(() =>
    colorHinges?.options?.find((item) => item.value === selectedColorValue)
      ?.label ?? "", [selectedColorValue, colorHinges]);

  const currentSize = useMemo(() =>
    sizeHinges?.options?.find((item) => item.value === selectedSizeValue)
      ?.label ?? "", [selectedSizeValue, sizeHinges]);
      ...
      {options.map((item, index) => (
        <label
          ...
          <div
            ...
            className="size-full overflow-hidden rounded-sm bg-cover bg-center bg-no-repeat"></div>
        </label>
      ))}
    </RadioGroup>
  );
